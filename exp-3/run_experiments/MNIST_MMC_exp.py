...
def t_print(s, t):
    """Prints a string followed by the time."""
    print(s+".... {:2.2f} s".format(t))
    ...
    def save_record(self, fold_records):
        ...
        with open(filename, "wt") as f:
            f.write(self.status + " " + str(self.n) + " " + str(self.B) + " "
                    + str(self.test_obj) + "  " + str(self.test_cons) + " "
                    + str(int(self.t)) + "\n")
...
class MMC:
    ...
    def project_A(self):
        """Projects the matrix A repeatedly."""
        ...
        while  n_neg_eigvals > 0 or not  self.constr <= 1 + self.tol:
            if self.verbose > 1:
                t_print("n_neg_eigvals={:2}/constraint={:4.2f}".format(int(n_neg_eigvals),
                                                                       self.constr),
    ...
    def project_A_on_C1(self, constr):
        """Projects the solution on the set that satisfies the constraint."""
        if constr <= 1:
            self.A = self.A
        else:
    ...
    def project_A_on_C2(self):
        """Projects the solution on the set of matrices with positive eigenvalues."""
        eigvals, eigvecs = np.linalg.eigh(self.A)
    ...
    def fit(self, n_it=5000):
        """Does n_it iterations to fit the MMC model."""
...
def main():
    ...
    if args.ratio is None and args.type_exp == "incomplete":
        raise ValueError("You did not supply a ratio for B/N while asking "
                         + "for incomplete U-stats experiments...")
    ...
    if type_exp == "complete":
        ...
        iterators = {"pos": CompPosIterator,
                     "neg": lambda X, y: ([X[i] for i in i_s] for i_s in selected_elems)}
        tuple_iter_neg = True
    ...
    mmc = MMC(X_train_pca_red, y_train, iterators, tuple_iter_neg=tuple_iter_neg,
              init_time=init_time)
    mmc.fit(n_it=5000)

    record = ExpRecord(n_choice, type_exp, B, mmc.A)
    record.save_time(time.time() - begin_time)
    record.save_test_obj(X_test_pca_red, y_test)
